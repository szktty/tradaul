...
### Running Official Lua Test Suite

The official Lua 5.4.6 test suite is located in the `lua-5.4.6-tests` directory. To run these tests:
...
3. **Run individual test files**:
   ```bash
   ../bin/tradaul all.lua    # Main test runner
   ../bin/tradaul basic.lua   # Basic language tests
   ../bin/tradaul strings.lua # String library tests
   ../bin/tradaul math.lua    # Math library tests
   ```

4. **Expected Results**:
   - Tests for implemented features should pass
   - Tests for unimplemented APIs will fail with clear error messages
   - Some tests may need to be skipped due to missing features (I/O, debug, etc.)

5. **Debugging Test Failures**:
   - Use `--debug` flag for verbose output: `../bin/tradaul --debug test.lua`
   - Compare behavior with standard Lua 5.4 interpreter
   - Check test source code to understand expected behavior
...
- `-b, --bytecode` - Show generated bytecode and exit
...
# Show generated bytecode without execution
bin/tradaul --bytecode script.lua
bin/tradaul --bytecode -e "print('Hello')"

# Enable debug mode
bin/tradaul --debug script.lua
```

### Debugging with Fixed Files

When debugging Lua code with Claude Code, use fixed debug files instead of command-line one-liners to avoid manual acceptance prompts in VSCode:

1. **Create a debug file**: Use `debug.lua` in the project root for temporary debugging code
2. **Edit and run**: Modify the contents of `debug.lua` and execute with `bin/tradaul debug.lua`
3. **Benefits**: The command `bin/tradaul debug.lua` remains constant, allowing it to be whitelisted in `settings.local.json` for automatic execution

Example workflow:
```bash
# Write debug code to fixed file
echo 'print("Debug test")' > debug.lua

# Run the debug file (this command can be auto-accepted)
bin/tradaul debug.lua

# Modify and re-run as needed
```

**Note**: The `debug.lua` file is gitignored and should not be committed. This approach avoids the need for manual acceptance when testing small code snippets, as command-line arguments containing code (like `-e "code"`) cannot be easily whitelisted.
